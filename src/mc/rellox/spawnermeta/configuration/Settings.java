...
public final class Settings {
	...
	protected void reload0() {
		...
		stacking_price.load();
		stacking_spawner_limit = file.getInteger("Modifiers.stacking.spawner-limit");
		...
		Set<String> keys = CF.s.keys("Modifiers.breaking.permissions");
		chance_permissions.clear();
		if(keys.isEmpty() == false) {
			for(String key : keys) {
				String p = "spawnermeta.breaking.permission." + key;
				double c = file.getDouble("Modifiers.breaking.permissions." + key);
		...
		command_view = file.getString("Commands.spawner-view");
		command_shop = file.getString("Commands.spawner-shop");
		command_drops = file.getString("Commands.spawner-drops");
		
		spawner_version = file.getInteger("Spawner-version");
